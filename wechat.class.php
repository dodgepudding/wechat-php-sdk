...
class Wechat
{
    ...
    const TEMPLATE_GET_ALL_PRIVATE_URL = '/template/get_all_private_template?';
    const TEMPLATE_DELETE_URL = '/template/del_private_template?';
...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @return bool
     */
    public function deleteTemplate($tpl_id){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_DELETE_URL.'access_token='.$this->access_token,self::json_encode(array('template_id'=>$tpl_id)));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
